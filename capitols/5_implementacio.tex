...
Pel que fa als estils, s'ha utilitzat \textbf{SCSS modular}. S'han definit variables globals per mantenir la consistència visual i s'ha aplicat l'estratègia de detecció de canvis \texttt{OnPush} per garantir que la interfície només es refresqui quan realment hi hagi canvis d'estat significatius, millorant enormement el rendiment de la CPU al dispositiu de client.
...
\begin{enumerate}
  ...
  \item \textbf{Capa Física (Recursos - \texttt{/resources}):} Exposa el maquinari realment connectat o llicenciat en l'equip en temps d'execució mi\textbf{mitjançant} un conjunt d'APIs REST complementàries (ex: \textit{"aquest equip disposa d'un mòdul físic amb 8 entrades instal·lades"}).
...
\begin{enumerate}
  ...
  \item Recorre l'esquema del node en paral·lel. Para cada propietat que no estigui definida en la persistència, hi injecta automàticament el seu valor per defecte (\texttt{\_default}).
...
Si el recurs d'edició està ocupat, el servidor respon amb un codi \texttt{HTTP 409 Conflict} aportant el nom de l'usuari i la IP que té el bloqueig, obligant l'aplicació Angular a bloquejar la vista actual com a "Només lectura" de forma instantània (Figura \ref{fig:seq_config_lock}).
...
\begin{figure}[htbp]
  ...
  \begin{verbatim}
  +---------------------------------------------------------------------------------+
  |                            CAPA DE MAQUINARI I SENYALS                          |
  |  [Sensor / Camara IP] --(Hardware Interrupt)--> [D-Bus com.lanaccess.sysevents]  |
  +---------------------------------------------------------------------------------+
                                           |
                                           v (Signal "ALARM_NOTIFY")
  +---------------------------------------------------------------------------------+
  |                           MICROSERVEI "LA-CORE-ALARM"                           |
  |  [dbus_listen_alarms_thread]                                                    |
  |            |                                                                    |
  |            v (Enqueue)                                                          |
  |      [global_alarm_queue] (SafeQueue)                                           |
  |            |                                                                    |
  |            v (Dequeue & Process)                                                |
  |  [db_client_thread] --(gRPC call Execute/Query)----------------+                |
  +----------------------------------------------------------------|----------------+
                                                                   |
                                                                   v (unix:// socket)
  +----------------------------------------------------------------|----------------+
  |                           MICROSERVEI "LA-DB-ENGINE"           |                |
  |  [DatabaseServiceImpl] <---------------------------------------+                |
  |            |                                                                    |
  |            v (Prepare & Exec SQLite3)                                           |
  |      [SQLite3 Engine]                                                           |
  |            |                                                                    |
  |            v (Intercept Read/Write Operations)                                  |
  |      [Remote VFS Driver] --(Unix Domain Socket /tmp/io.sock)---+                |
  +----------------------------------------------------------------|----------------+
                                                                   |
                                                                   v (IoRequest)
  +----------------------------------------------------------------|----------------+
  |                           MICROSERVEI "LA-VIDEO-ENGINE"        |                |
  |  [IO Server Listener] <----------------------------------------+                |
  |            |                                                                    |
  |            v (Direct Block I/O Operations)                                      |
  |      [Hardware Disk Driver] ------> [Fisic Flash/HDD Storage]                    |
  +---------------------------------------------------------------------------------+
                                                     |
  +--------------------------------------------------+ (Atomic Write OK)
  |
  v (Enqueue as EmissionData)
  +---------------------------------------------------------------------------------+
  |                  NOTIFICACIÓ DE TORNADA I EMISSIÓ CAP A LA WEB                  |
  |  [global_emission_queue]                                                        |
  |            |                                                                    |
  |            v                                                                    |
  |  [ws_emit_alarms_thread] ----(JSON string over libwebsockets)----> [Proxy NGINX] |
  |                                                                         |
  |                                                                         v (WSS)
  |                                                              [Frontend Angular] |
  +---------------------------------------------------------------------------------+
...
\begin{itemize}
  ...
  \item \textbf{Restabliment de Fàbrica (Factory Reset):} Permet retornar la base de dades a l'estat d'origen. Per evitar la pèrdua accidental de dades o atacs de tipus CSRF, s'imposa un mètode securitzat en d'una sola vegada: el client demana primer un token d'autorització d'escriptura de canvis d'un sol ús (\textit{Token de seguretat}) a la-core-legacy, i només si és vàlid, es pot processar la crida destructiva.
\end{itemize}
