...
L'aplicació ha d'oferir un cicle complet de gestió: autenticació segura, monitorització de càmeres en directe, cerca de gravacions històriques, gestió d'alarmes, configuració avançada del dispositiu i manteniment remot (\textit{firmware}, llicències, logs). Tota la funcionalitat ha de ser operable des d'un navegador modern, sense cap extensió externa ni infraestructura de servidor addicional més enllà del propi gravador NXT (vegeu \ref{sec:solucioproposada}).

\subsection{Obstacles detectats}
S'han identificat els reptes tècnics principals que el projecte ha hagut de superar:

\begin{itemize}
  \item \textbf{Suport irregular de H.265 als navegadors:} La descodificació hardware de H.265 (HEVC) no és universal als navegadors: depèn de la disponibilitat de decodificació hardware al terminal client i de restriccions de patents. Afecta \textbf{el Projectista} (disseny de la lògica de \textit{fallback} automàtic a H.264) i l'\textbf{Usuari final} (potencial increment de latència en terminals sense suport hardware).
