...
Per tal de garantir l'operativa del sistema, la plataforma interactua amb quatre serveis crítics executant-se en l'entorn \textit{embedded}:
\begin{itemize}
    \item \textbf{la-core-legacy:} És el servei encarregat de la lògica de negoci heretada i la gestió de la configuració base del sistema. La interfície web hi comunica mitjançant peticions REST per validar credencials i obtenir els esquemes de configuració.
    \item \textbf{la-video-engine:} Actua com el node central de processament multimèdia. Gestiona la negociació de les sessions WebRTC i la fragmentació de vídeo per a HLS. La web hi interacciona per establir els canals de visualització de baixa latència.
    \item \textbf{la-core-alarm:} Servei especialitzat en la monitorització de sensors i lògiques de seguretat. Publica esdeveniments en temps real al bus de dades intern, que posteriorment són transmesos al \textit{frontend} mitjançant WebSockets.
    \item \textbf{la-db-engine:} Gestiona la capa de persistència mitjançant SQLite. S'encarrega de l'emmagatzematge indexat de logs i l'historial d'alarmes, garantint que la web pugui realitzar cerques ràpides sobre milers de registres.
\end{itemize}
