...
L'especificació de requisits és la pedra angular del projecte \textit{Web Core View}. En tractar-se d'un sistema de gestió per a dispositius de seguretat crítica, la definició de les funcionalitats no només s'ha centrat en l'experiència d'usuari (UX), sinó també en la robustesa de la comunicació asíncrona i el desacoblament entre el maquinari i la interfície.
...
Per il·lustrar com els operadors de seguretat apliquen les històries d'usuari en la resposta a un incident real, es presenta el diagrama de User Journey (Figura \ref{fig:user_journey}).
...
\begin{itemize}
  \item \textbf{RF-3.1 Interpretació de l'Esquema:} El sistema ha de processar un fitxer JSON de definició. Per tal d'il·lustrar la capacitat del motor dinàmic (\textit{schema-driven}) de desacoblar la interfície del frontend respecte de la lògica de baix nivell del gravador, es presenta a continuació un fragment d'esquema JSON de configuració avançada real enviat pel servei de vídeo. Aquest esquema defineix els tipus de transport admesos pel maquinari de visualització d'una càmera IP física (per estructura d'enumeració) i la restricció d'adreces de multidifusió (multicast) mitjançant expressions regulars (\textit{regex}) que el motor del frontend compilarà i validarà dinàmicament.
\end{itemize}

\begin{lstlisting}[basicstyle=\footnotesize\ttfamily, breaklines=true, language=JavaScript, caption={Exemple d'esquema avançat JSON de metadades per a IP Video amb restriccions regex.}]
{
  ...
  "media.video.{streamId}.multicast.port": {
    ...
    "_min": 1024,
    "_max": 65535,
    "_description": "Port de destinació per a la transmissió Multicast de dades de vídeo",
    "_default": 30010
  }
}
\end{lstlisting}
