...
\begin{description}
  \item[\textbf{US-09: Consulta i renovació de llicència}] \hfill \\
        \textbf{Com a} Tècnic, \textbf{vull} consultar l'estat de la llicència del dispositiu i renovar-la quan sigui necessari, \textbf{per tal de} garantir la continuïtat del servei sense interrupcions.

  \item[\textbf{US-10: Actualització de firmware}] \hfill \\
        \textbf{Com a} Tècnic, \textbf{vull} actualitzar el firmware del dispositiu des de la interfície web, \textbf{per tal de} mantenir el sistema al dia amb les darreres millores i correccions de seguretat.
  ...
  \item[\textbf{US-12: Accés a la documentació del sistema}] \hfill \\
        \textbf{Com a} Tècnic, \textbf{vull} tenir accés a tota la documentació tècnica del sistema des de la interfície web, \textbf{per tal de} resoldre dubtes i consultar especificacions sense necessitat de recursos externs.
...
\begin{itemize}
  \item \textbf{RF-3.1 Interpretació de l'Esquema:} El frontend ha de processar l'esquema JSON rebut del backend i renderitzar dinàmicament el component visual adequat per a cada paràmetre (camp de text, llista desplegable, \textit{slider} numèric), aplicant les restriccions de tipus, rang i format (\textit{regex}) definides a l'esquema sense requerir canvis al codi del frontend.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|p{10cm}|}
    ...
    \textbf{Descripció}                     & L'usuari introdueix les seves credencials per obtenir accés al sistema. El backend les valida, estableix una cookie de sessió segura, i la interfície inicialitza l'estat de l'usuari.                           \\ \hline
    ...
    3. El frontend envia les credencials al servei d'autenticació del backend. \newline
    4. El backend valida les credencials i estableix una cookie de sessió segura, inaccessible des del codi client. \newline
    ...
    3a. Si les credencials són incorrectes, el sistema mostra un avís d'accés denegat. L'usuari pot reintentar.                  \\ \hline
    \textbf{Postcondició}                   & L'usuari té accés a les funcionalitats del sistema d'acord amb el seu rol. La sessió es manté activa automàticament mentre el navegador roman obert.                             \\ \hline
  \end{tabular}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|p{10cm}|}
    ...
    3. El sistema valida el format i la mida del fitxer abans de procedir. \newline
    4. S'inicia la transferència del fitxer al dispositiu. \newline
    5. El sistema mostra el progrés de la transferència en temps real. \newline
    6. Un cop finalitzada la transferència, el gravador aplica el nou firmware i es reinicia.                                                                                 \\ \hline
    \textbf{Flux Alternatiu}                &
    5a. Si la connexió es talla durant la transferència, el sistema informa de l'error i permet reintentar l'operació.          \\ \hline
    \textbf{Postcondició}                   & El dispositiu es reinicia amb el nou firmware. La interfície mostra una pantalla d'espera fins que el dispositiu torna a estar en línia. Si l'actualització falla, el dispositiu recupera automàticament la versió anterior de firmware per garantir la continuïtat del servei.                                                    \\ \hline
  \end{tabular}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|l|p{10cm}|}
    ...
    \textbf{Precondicions}                  & Usuari autenticat amb permisos de visualització. El gravador disposa de gravacions per a la data sol·licitada. La llicència del dispositiu permet l'accés a la reproducció d'historials. \\ \hline
    \textbf{Flux Principal}                 &
    ...
    5. El reproductor inicia la càrrega del fragment de vídeo corresponent al punt seleccionat. \\ \hline
    \textbf{Flux Alternatiu}                &
...
\begin{itemize}
  \item \textbf{Seguretat:} Per evitar atacs \textit{Cross-Site Scripting} (XSS) i \textit{Cross-Site Request Forgery} (CSRF), no s'utilitzen cookies llegibles per JavaScript ni el \textit{LocalStorage} del navegador per emmagatzemar dades de sessió. La sessió es gestiona mitjançant cookies segures inaccessibles des del codi client, de manera que cap \textit{script} de la pàgina pot llegir ni modificar les credencials. L'estat d'autenticació al \textit{frontend} es manté únicament en memòria.
