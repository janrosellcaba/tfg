...
\begin{itemize}
  \item \textbf{Seguretat:} Per evitar atacs \textit{Cross-Site Scripting} (XSS) i \textit{Cross-Site Request Forgery} (CSRF), no s'utilitzen cookies llegibles per JavaScript (\texttt{document.cookie}) ni el \textit{LocalStorage} del navegador per emmagatzemar dades de sessió. La sessió es gestiona amb cookies \textit{HttpOnly} i \textit{Secure} controlades exclusivament per NGINX (inaccessibles des de codi JavaScript) i un Signal privat en memòria al \textit{frontend} de zero-persistència: un recarregament de pàgina tanca la sessió, comportament intencionat per a entorns de seguretat industrial.
